...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        $email = $_POST['email'];
        $password = !empty($_POST['password']) ? $_POST['password'] : $siswa['password'];
        ...
        if ($nis !== $siswa['nis']) {
            $check_sql = "SELECT COUNT(*) FROM siswa WHERE nis = :nis AND id != :id";
            $check_stmt = $conn->prepare($check_sql);
            $check_stmt->execute(['nis' => $nis, 'id' => $id]);
        ...
        $foto_profil = $siswa['foto_profil']; // Default to current photo
        ...
        if (isset($_FILES['foto_profil']) && $_FILES['foto_profil']['error'] === 0) {
            ...
            if (move_uploaded_file($_FILES['foto_profil']['tmp_name'], $target_file)) {
                ...
                if ($siswa['foto_profil'] !== 'assets/default/photo-profile.png' && file_exists('../../' . $siswa['foto_profil'])) {
        ...
        $stmt->execute([
            ...
            'description' => "Admin mengedit data siswa: $nama_lengkap ($nis)"
        ]);
                    ...
                    <form method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-6 text-center">
                            <div class="relative w-28 h-28 md:w-32 md:h-32 mx-auto">
                                <img id="preview-image" src="../../<?= $siswa['foto_profil'] ?>"
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                            ...
                            <div class="mb-2 md:mb-0">
                                ...
                                <select id="jurusan" name="jurusan" required
                                    class="w-full bg-gray-800/50 border border-gray-700 rounded-lg px-3 py-3 md:py-2 text-white focus:outline-none focus:border-purple-500 touch-target">
                                    <option value="BR" <?= $siswa['jurusan'] === 'BR' ? 'selected' : '' ?>>KURMER</option>
                                    <option value="MP" <?= $siswa['jurusan'] === 'MP' ? 'selected' : '' ?>>MIPA</option>
                                </select>
                            </div>
